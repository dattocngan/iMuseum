            ...
            {!isLoading &&
                <div className="row">
                    ...
                    <div className="col-md-8">
                        <h5><span className="fw-bold">Trạng thái:</span> <span className={item.status ? 'text-success' : 'text-danger'}>{item.status ? 'Đã được duyệt' : 'Chưa được duyệt'}</span></h5>
                        <form className={`row g-3 needs-validation ${
                            ...
                            <div className="col-md-4">
                                ...
                                <input disabled={item.status} ref={nameInputRef} type="text" className="form-control" style={{outline: 'none'}} defaultValue={item.name} id="name" required />
                            ...
                            <div className="col-md-4">
                                ...
                                <input disabled={item.status} ref={originalInputRef} defaultValue={item.original} type="text" className="form-control" id="original" />
                            </div>
                            <div className="col-md-4">
                                <label htmlFor="dimension" className="form-label">Kích thước:</label>
                                <input disabled={item.status} ref={dimensionInputRef} defaultValue={item.dimension} type="text" className="form-control" id="dimension" />
                            </div>
                            <div className="col-md-3">
                                <label htmlFor="weight" className="form-label">Cân nặng (kg):</label>
                                <input disabled={item.status} ref={weightInputRef} defaultValue={item.weight} step="0.01" type="number" className="form-control" id="weight" />
                            </div>
                            <div className="col-md-3">
                                ...
                                {!item.status &&
                                    <>
                                        <select disabled={item.status} ref={ageInputRef} className="form-select" id="age" defaultValue={item.ageId || ''} required>
                            ...
                            <div className="col-md-3">
                                ...
                                {!item.status &&
                                    <>
                                        <select ref={materialInputRef} className="form-select" id="material" defaultValue={item.materialId || ''} required>
                            ...
                            <div className="col-md-3">
                                ...
                                <input disabled={item.status} ref={dateInputRef} type="date" className="form-control" id="date" defaultValue={item.collected_date || '2015-12-31'}/>
                            </div>
                            {!item.status &&
                                <>
                            ...
                            <div className="col-12">
                                ...
                                <textarea disabled={item.status} ref={descriptionInputRef} className="form-control" defaultValue={item.description} id="description" rows="7"></textarea>
                            </div>
                            {!item.status &&
                                <div className="col-12">
                            ...
                            <>
                                    ...
                                    <div key={image.image_id} className="col-md-3 mt-5">
                                        {!item.status &&
                                            <div className="form-check">
                                                <input className="form-check-input" type="checkbox" onClick={addDeleteImage.bind(null, image.image_id)} />
                                            </div>
                                        }
                                ...
                                {!item.status &&
                                    <div className="col-md-12 mt-4">
                                        <button className="btn btn-danger" disabled={!deleteImageList.length} onClick={deleteImagesHandler}>Xóa các ảnh đã chọn</button>
                                    </div>
                                }
                            </>
                        }
