...
function Verify() {
    ...
    return (
        <div className={classes.layout}>
            <div className="card">
                {invalidForm && <span className={`text-center mt-2 ${classes.invalid}`}>{invalidForm}</span>}
                <article className="card-body">
                    <h4 className="card-title mb-4 mt-1">iMuseum</h4>
                    <form onSubmit={submitFormHandler}>
                        <div className="form-group">
                            <label>Vui lòng nhập mã xác thực:</label>
                            <input className="form-control" type="text" ref={otpInputRef} maxLength={4} />
                            {!!errorOTP && <p className={classes.invalid}>OTP chứa 4 chữ số</p>}
                        </div>
                        <div className="form-group">
                            <button type="submit" className="btn btn-primary btn-block">Xác thực</button>
                        </div>
                        <div className="form-group d-flex">
                            <button disabled={refreshOTP} type="button" className="btn btn-success btn-block w-50" onClick={refreshOTPHandler}>Gửi lại mã</button>
                            {refreshOTP && <Countdown date={Date.parse(refreshTime) + 1000*60*3} onComplete={completeHandler} renderer={renderer} />}
                        </div>
                        {refreshMessage && <div className="form-group">
